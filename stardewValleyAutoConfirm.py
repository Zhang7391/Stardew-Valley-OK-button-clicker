...
def foreach_window(testHwnd, lParam):
    if user32.IsWindowVisible(testHwnd):
        length = user32.GetWindowTextLengthW(testHwnd)
        buff = ctypes.create_unicode_buffer(length + 1)
        user32.GetWindowTextW(testHwnd, buff, length + 1)
        title = buff.value
        if title.startswith("Stardew Valley"):  # Only match the beginning of the title
            hwnd.append(testHwnd)
            return False  # Stop enumeration when found
    return True

# Define the callback function prototype
user32.EnumWindows(ctypes.WINFUNCTYPE(ctypes.c_bool, ctypes.c_int, ctypes.c_int)(foreach_window), 0)
...
def click(x, y):
    ...
    for i in range(steps):
        user32.SetCursorPos(int(start_x + dx*i), int(start_y + dy*i))
        time.sleep(delay)
